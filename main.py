...
with tab1:
    ...
    thr = st.slider("RSIしきい値", 10, 85, 60, key="s1")
    if st.button("🚀 スキャン開始", key="b1"):
        st.session_state.hits = []
        bar = st.progress(0); status = st.empty()
        for i, (t, n) in enumerate(JPX400_DICT.items()):
            bar.progress((i+1)/len(JPX400_DICT))
            status.text(f"取得中: {t} {n}")
            try:
                # 1銘柄ずつ確実に。MultiIndexを避けるため
                df = yf.download(t, period="3mo", progress=False)
                if df.empty: continue
                # Close列を確実に抽出
                close = df['Close'].iloc[:, 0] if isinstance(df['Close'], pd.DataFrame) else df['Close']
                rsi = get_rsi(close.dropna())
                val = rsi.tail(5).min()
                if val <= thr:
                    name_full = f"{t} {n}"
                    st.session_state.hits.append(name_full)
                    st.session_state.reasons[t] = f"5日RSI低迷({val:.1f})"
                    st.write(f"✅ ヒット: {name_full}")
            except: continue
        status.empty(); bar.empty()
        st.rerun() # リストに反映させるため再起動
    
    sel1 = st.multiselect("監視に追加（ここに入った銘柄を保存）", all_options, default=st.session_state.hits, key="m1")
    if st.button("💾 保存（タブ1）", key="sv1"): save_list(sel1)
...
with tab2:
    ...
    if st.button("🔍 複合スキャン開始", key="b2"):
        ...
        for i, (t, n) in enumerate(JPX400_DICT.items()):
            ...
            try:
                ...
                close = df['Close'].iloc[:, 0] if isinstance(df['Close'], pd.DataFrame) else df['Close']
                rsi_v = get_rsi(close.dropna()).iloc[-1]
                rci_v = get_rci(close.dropna()).iloc[-1]
    ...
    sel2 = st.multiselect("監視に追加", all_options, default=st.session_state.get('hits2', []), key="m2")
    if st.button("💾 保存（タブ2）", key="sv2"): save_list(sel2)

with tab3:
    st.header("☀️ 現在の監視リスト")
    if st.button("🗑️ 銘柄を全削除", type="primary", key="del"):
        save_list([]); st.rerun()
