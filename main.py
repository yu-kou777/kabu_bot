...
with tab1:
    if st.button("日足RSI 20以下を抽出"):
        st.session_state.found = JPX400_SAMPLE # 簡易実装
    ...
    if 'found' in st.session_state:
        ...
        for t in st.session_state.found:
            ...
            if df is not None:
                ...
                # 垂直並び表示
                with st.expander(f"【{t}】 {trend}", expanded=True):
                    ...
                    if not st.button(f"削除 {t}"): final.append(t)
...
with tab2:
    if os.path.exists(WATCHLIST_FILE):
        ...
        st.write(f"現在 {len(watchlist)} 銘柄を監視中...")
        if st.button("監視スタート"):
            for t in watchlist:
                df = get_stock_data(t)
