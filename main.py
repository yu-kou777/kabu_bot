...
st.set_page_config(page_title="Jack株AI：スキャナー改良版", layout="wide")

def get_jst_now():
    return datetime.now(timezone(timedelta(hours=9)))

# RSI計算 (データを2ヶ月分取得して精度を向上)
...
with tab1:
    ...
    if 'last_scan_hits' not in st.session_state: st.session_state.last_scan_hits = []

    col1, col2 = st.columns(2)
    with col1:
        st.subheader("① 低RSI検索")
        low_rsi_val = st.slider("しきい値", 10, 70, 40) # 40だと出ない場合、50〜60でテスト可能
        if st.button("🔍 低RSI銘柄をスキャン"):
            st.session_state.last_scan_hits = [] # リセット
            rsi_data = get_latest_rsi(list(JPX400_DICT.keys()))
            for t, v in rsi_data.items():
                if v <= low_rsi_val: 
                    st.session_state.temp_watchlist[t] = "低RSI銘柄"
                    st.session_state.last_scan_hits.append(f"{t} {JPX400_DICT[t]} (RSI:{v:.1f})")
            if not st.session_state.last_scan_hits:
                st.warning(f"RSI {low_rsi_val} 以下の銘柄は見つかりませんでした。")
            else:
                st.success(f"{len(st.session_state.last_scan_hits)}件見つかりました！")
    ...
    with col2:
        ...
        if st.button("🔍 異常過熱をスキャン"):
            ...
            for t, v in rsi_data.items():
                if v >= 75: 
                    st.session_state.temp_watchlist[t] = "RSI高騰"
                    st.session_state.last_scan_hits.append(f"{t} {JPX400_DICT[t]} (RSI:{v:.1f})")
                elif v <= 15: 
                    st.session_state.temp_watchlist[t] = "RSI下落(極)"
                    st.session_state.last_scan_hits.append(f"{t} {JPX400_DICT[t]} (RSI:{v:.1f})")
            if not st.session_state.last_scan_hits:
    ...
    # 見つかった銘柄をその場でリスト表示
    if st.session_state.last_scan_hits:
        with st.expander("🔍 スキャン結果の確認", expanded=True):
            for hit in st.session_state.last_scan_hits:
                st.write(f"✅ {hit}")
    ...
    selected_full = st.multiselect("監視リストに追加・確認（ここに入っている銘柄が保存されます）", options, default=current_hits)
    
    col_save, col_clear = st.columns(2)
    with col_save:
        if st.button("💾 この内容で監視を確定保存", use_container_width=True):
            final_list = []
            ...
            st.session_state.last_scan_hits = []
