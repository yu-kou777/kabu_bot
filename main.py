...
st.set_page_config(page_title="Jack株AI：完全統合パネル", layout="wide")

# --- メモリの初期化（干渉を防ぐため別々の変数名に） ---
...
# --- UI ---
tab1, tab2, tab3 = st.tabs(["🔍 5日RSI検索", "📊 RCI複合分析", "☀️ 監視状況"])
options = [f"{k} {v}" for k, v in JPX400_DICT.items()]

with tab1:
    st.header("🌙 直近5日間のRSI低迷を探す")
    thr1 = st.slider("RSIしきい値", 10, 80, 60, key="slider1")
    if st.button("🚀 5日検索スキャン開始", key="btn1"):
        new_hits = []
        ...
        st.session_state.hits_5d = new_hits
        status.empty(); bar.empty()
        st.rerun()
    
    sel1 = st.multiselect("監視に追加（5日検索結果）", options, default=st.session_state.hits_5d, key="ms1")
    if st.button("💾 保存（5日検索結果）", key="sv1"): save_list(sel1)

with tab2:
    st.header("📊 RSI × RCI 複合スキャナー")
    col_rsi, col_rci = st.columns(2)
    rsi_target = col_rsi.slider("RSI条件 (以下)", 20, 70, 45, key="rsi_target")
    rci_target = col_rci.slider("RCI条件 (以下)", -100, 0, -70, key="rci_target")
    
    if st.button("🔍 複合スキャン開始", key="btn2"):
        new_hits_comp = []
        ...
        for i, (t, n) in enumerate(JPX400_DICT.items()):
            ...
            try:
                ...
                rsi_v = calculate_rsi(close_d).iloc[-1]
                rci_v = calculate_rci(close_d).iloc[-1]
                
                if rsi_v <= rsi_target and rci_v <= rci_target:
                    name_full = f"{t} {n}"
                    new_hits_comp.append(name_full)
                    st.session_state.reasons[t] = f"複合大底(RSI:{rsi_v:.1f}, RCI:{rci_v:.1f})"
            except: continue
        st.session_state.hits_comp = new_hits_comp
        status2.empty(); bar2.empty()
        st.rerun()
    
    sel2 = st.multiselect("監視に追加（複合結果）", options, default=st.session_state.hits_comp, key="ms2")
    if st.button("💾 保存（複合結果）", key="sv2"): save_list(sel2)

with tab3:
    st.header("☀️ 現在の監視リスト")
    if st.button("🗑️ 全削除", type="primary"):
        st.session_state.hits_5d = []; st.session_state.hits_comp = []
        save_list([]); st.rerun()
