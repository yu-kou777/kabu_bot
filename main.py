...
TEMP_HITS_FILE = "temp_hits.json" # スキャン結果を一時保存するファイル
...
st.set_page_config(page_title="Jack株AI：銘柄固定版", layout="wide")
...
with tab1:
    ...
    sel = st.multiselect("監視リスト（スキャン結果が自動で入ります）", options, default=current_hits, key="select_box")
    ...
    if os.path.exists(WATCHLIST_FILE):
        with open(WATCHLIST_FILE, 'r', encoding='utf-8') as f:
            ...
            for item in watch_data:
                st.write(f"🔹 **{item['ticker']} {item.get('name')}** (登録日: {item.get('at')})")
