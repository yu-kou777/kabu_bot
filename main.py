...
with tab1:
    ...
    thr = st.slider("RSIしきい値", 10, 80, 60, key="slider1")
    if st.button("🚀 RSIスキャン開始", key="btn1"):
        st.session_state.hits_tab1 = [] # リセット
        ...
        for i, t in enumerate(tickers):
            ...
            try:
                ...
                rsi_s = calculate_rsi(close_s.dropna())
                m = rsi_s.tail(5).min()
                if m <= thr:
        ...
        st.rerun() # スキャン完了後に画面を再描画してリストに反映
    
    # defaultにセッション変数を指定することで、銘柄が消えなくなります
        ...
        for i, t in enumerate(tickers):
            ...
            try:
                ...
                if (rsi <= 35 and rci <= -80) or (rsi >= 75 and rci >= 80):
                    ...
                    st.session_state.reasons[t] = f"複合(RSI:{rsi:.1f}, RCI:{rci:.1f})"
            except: continue
...
with tab3:
    st.header("☀️ 監視リスト管理")
    if st.button("🗑️ 全銘柄を削除", type="primary", key="clear"):
        save_list([]); st.rerun()
